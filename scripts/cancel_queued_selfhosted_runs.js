...
async function listWorkflows(owner, repo, token) {
  const { json } = await requestJson({ method: 'GET', pathname: `/repos/${owner}/${repo}/actions/workflows?per_page=100`, token });
...
async function listRuns(owner, repo, workflowId, token, status) {
  ...
  const runs = [];
  while (true) {
    const { json } = await requestJson({ method: 'GET', pathname: `/repos/${owner}/${repo}/actions/workflows/${workflowId}/runs?status=${status}&per_page=100&page=${page}`, token });
