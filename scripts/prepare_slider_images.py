...
Автоматически обрабатывает изображения согласно требованиям:
1. Поворачивает для читаемости текста
2. Удаляет лишний фон
3. Приводит к единому размеру
4. Сохраняет с русскими именами
"""
...
def auto_rotate_image(image):
    ...
    try:
        ...
        # Дополнительная логика: если изображение шире, чем выше, возможно нужно повернуть
        if image.width > image.height * 1.5:
            # Проверяем, есть ли текст, который нужно читать горизонтально
            # Для простоты пока не поворачиваем, но можно добавить OCR
            pass
...
def main():
    ...
    try:
        ...
        logger.error("Модуль rembg не установлен. Установите: pip install rembg")
        sys.exit(1)
    
    logger.info("Начинаю обработку изображений...")
